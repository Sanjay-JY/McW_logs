...
__global__ void lookuptable_kernel(float2 *in1, float2 *in2, int2 *d_lookup, float2 *d_xxresult, float2 *d_yyresult, float2 *d_xyresult, float2 *d_yxresult) {
    ...
    if (rowIdx < 16640) {
        ...
        d_xxresult[rowIdx].x = sum.x;
        d_xxresult[rowIdx].y = sum.y;
        ...
        for (int colIdx = 1; colIdx < 1003; colIdx++) {
            int2 current = d_lookup[rowIdx * 1003 + colIdx];
                if(current.x==-1) break;
            float2 product;
            product.x = (in2[current.y].x * in2[current.x].x) - (in2[current.y].y *-1.0* in2[current.x].y);
            product.y = (in2[current.y].x * -1.0*in2[current.x].y) + (in2[current.y].y * in2[current.x].x);
              //printf("%f\t%f\t%d\n",product.x,product.y,rowIdx);
            product.y = -1.0*product.y;
            sum.x += product.x;
            sum.y += product.y;
        ...
        d_yyresult[rowIdx].x = sum.x;
        d_yyresult[rowIdx].y = sum.y;
        ...
        d_xyresult[rowIdx].x = sum.x;
        d_xyresult[rowIdx].y = sum.y;
        ...
        for (int colIdx = 1; colIdx < 1003; colIdx++) {
            int2 current = d_lookup[rowIdx * 1003 + colIdx];
                if(current.x==-1) break;
            float2 product;
            product.x = (in2[current.y].x * in1[current.x].x) - (in2[current.y].y *-1.0* in1[current.x].y);
            product.y = (in2[current.y].x * -1.0*in1[current.x].y) + (in2[current.y].y * in1[current.x].x);
              //printf("%f\t%f\t%d\n",product.x,product.y,rowIdx);
            product.y = -1.0*product.y;
            sum.x += product.x;
            sum.y += product.y;
        ...
        d_yxresult[rowIdx].x = sum.x;
        d_yxresult[rowIdx].y = sum.y;
    }
}
    ...
    for (int i = 0; i<inSize2; i++) {
        ...
        for (int ilag=0; ilag<nlag; ilag++) {
            ...
            else
            {
                ...
                //     printf("exp->%d\tlast->%d\tcidx->%d\tsize->%d\n",expIdx,(lookuptable[expIdx*1003].x),expIdx*1003 +(lookuptable[expIdx*1003].x),16640*1003* sizeof(int2));
                // }
                lookuptable[expIdx*1003 +(lookuptable[expIdx*1003].x)].x=i;
                lookuptable[expIdx*1003 + (lookuptable[expIdx*1003].x)].y=i+ilag;
                lookuptable[expIdx*1003].x= (lookuptable[expIdx*1003].x)+1;
