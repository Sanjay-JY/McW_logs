...
__global__ void cyclid_corr_accum_nlag_fast(float2 *in1, float2* in2, size_t size1, size_t size2, float2* out, int nlag, unsigned *phaseBins, int pfbChan, int numPfbChans, int iblock, int numPhaseBins, bool verbose,const int out_size) {
    if(out_size==1)
    {
        ...
        if((inIdx<size2) && (outIdx<outSz) && (yIdx<nlag)){
         ...
         // Declare shared memory for the histogram of type float2
    // __shared__ float2 histo_private[NUM_BINS];
    // // Initialize the shared histogram to zero
    // if (threadIdx.x < NUM_BINS)
    //     histo_private[threadIdx.x] = make_float2(0.0f, 0.0f);
    // __syncthreads();
    // // index into the data (in2)
    // int inIdx = blockIdx.x * blockDim.x + threadIdx.x;
    // // index into the lagged data partially (in1)
    ...
    // float2 tmp;
    // if ((inIdx < size2) && (outIdx < outSz) && (yIdx < nlag)) {
    //     // shift the in1 element, and multiply by in2 conjugate element
    //     int j = inIdx + yIdx;
    //     tmp.x = (in1[j].x * in2[inIdx].x) - (in1[j].y * -1.0 * in2[inIdx].y);
    //     tmp.y = (in1[j].x * -1.0 * in2[inIdx].y) + (in1[j].y * in2[inIdx].x);
    //     // we will want to take the C2R FFT of the conjugate of this
    //     tmp.y = -1.0 * tmp.y;
    //     // Update the shared histogram based on some computation
    //     atomicAdd(&histo_private[outIdx].x, tmp.x);
    //     atomicAdd(&histo_private[outIdx].y, tmp.y);
    //     __syncthreads();
    //     // Combine shared histograms into global histogram using atomic add
    //     if (threadIdx.x == 0) {
    //         for (int i = 0; i < NUM_BINS; ++i) {
    //             atomicAdd(&out[i].x, histo_private[i].x);
    //             atomicAdd(&out[i].y, histo_private[i].y);
    //         }
    //     }
    //     // Update the output based on your computation
    //     // atomicAdd(&out[outIdx].x, tmp.x);
    //     // atomicAdd(&out[outIdx].y, tmp.y);
    // }
    }
...
int test_cyclid_corr_accum(cycfold_struct *cs, unsigned *phaseBins, bool maxOccupancy, bool time) { 
    ...
    int phaseBinIdx, phaseBin, expIdx,maxi;
    float2 tmp, in1, in2;
    float2 *exp;
    ...
    const int out_size=profileSize*sizeof(float2);
    printf("%d\n",out_size);
    ...
    cyclid_corr_accum_nlag_fast<<<grids, threads>>>(in_gpu, in_gpu, inSize,  inSize2, out_gpu, nlag, phaseBins_gpu, ichan, nchan, iblock, nPhaseBins, verbose, 1);
