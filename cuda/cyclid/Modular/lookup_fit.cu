...
writing it into the global memory. It uses modified input similar to real-world data which
is made to fit in the shared memory and computes only one polaristion.

Link: https://bitbucket.org/assessmentmcw/cyclid/src/master/lookup_fit.cu
