/***************************************************************************************
A modified code where threads are launched for the output index and calculates the index
similar to the reference code. If the calculated index is similar to the threadIdx.x, it 
does the complex multiplication and writes the result in the output array. It uses input 
which is similar to the real-world data and computes only one polarisation.

Link: https://bitbucket.org/assessmentmcw/cyclid/src/master/threads_for_output.cu
